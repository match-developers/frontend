...
import HeartHugeIcon from 'match/frontend/assets/SVGs/IconHeartHuge';  // Import Heart icon
...
const MannerModal = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchMannerScore = async () => {
      try {
        const response = await axios.get(`http://your-backend-url/user/${userId}/statistics/`);  // Make API call to fetch user statistics
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.mannerContainer}>
        <HeartHugeIcon width={250} height={225} style={styles.icon} />  {/* Large heart icon */}
        <Text style={styles.ratingText}>{mannerScore !== null ? mannerScore.toFixed(1) : 'N/A'}</Text>  {/* Manner score displayed over the heart icon */}
      </View>

      {/* Manner score description */}
      <View style={styles.descriptionContainer}>
        <Text style={styles.descriptionText}>
          The Match Manners Rating is a reflection of how your club conducts itself on the app and in person. It is measured from fellow athletes reviewing your sportsmanship.
